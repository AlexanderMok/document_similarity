...
public class IDFLoader {
	...
	private void init(Path idfPath) {
		idfFreq = new HashMap<String, Double>();
		medianIDF = 0.0;
		setNewPath(idfPath);
	}

	protected void setNewPath(Path newIDFPath) {
		...
		if (path == null || !path.toString().equals(newIDFPath.toString())) {
			...
			try (BufferedReader br = Files.newBufferedReader(path);) {
				...
				System.out.println(
						String.format(Locale.getDefault(), "idf %s load finished, tot words:%d, time elapsed:%dms",
								newIDFPath.toString(), count, System.currentTimeMillis() - s));
